...
import { CheckCircle, XCircle, Clock, Trophy, RotateCcw, ChevronRight, ChevronLeft, HelpCircle, Star, AlertTriangle, Sparkles } from 'lucide-react';
...
export function QuizPlayer({ quizId, lessonTitle, onClose, onComplete }: QuizPlayerProps) {
    ...
    // Overlay backdrop
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1a130d]/65 backdrop-blur-md p-4 animate-fadeIn">
            <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[32px] border border-[#e9dccf] bg-[#fbf7f0] shadow-[0_30px_80px_rgba(29,18,10,0.35)]" style={{ animation: 'slideUp 0.3s ease-out' }}>
                {/* Header */}
                <div className="flex items-center justify-between shrink-0 border-b border-white/10 bg-[radial-gradient(circle_at_top_left,_rgba(96,165,250,0.25),_transparent_32%),linear-gradient(135deg,#101827,#1f3a5f)] px-6 py-4 text-white">
                    ...
                    <div className="flex items-center gap-3">
                        {state === 'taking' && (
                            <div className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-bold ${timeLeft < 60 ? 'bg-red-500/30 text-red-100 animate-pulse' : 'bg-white/15 text-slate-100'}`}>
                                <Clock size={16} />
                                {formatTime(timeLeft)}
                            </div>
                        )}
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {state === 'intro' && quiz && (
                        <div className="p-8 text-center">
                            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-[#dbeafe] shadow-inner">
                                <Sparkles size={38} className="text-[#2563eb]" />
                            </div>
                            <h3 className="mb-2 text-3xl font-bold text-[#111827]">{quiz.title}</h3>
                            <p className="mb-8 text-sm uppercase tracking-[0.22em] text-[#64748b]">Studio Knowledge Check</p>

                            <div className="mb-8 grid gap-4 md:grid-cols-3">
                                <div className="rounded-3xl border border-[#dbe4f0] bg-white px-4 py-5 shadow-sm">
                                    <div className="text-2xl font-bold text-[#1d4ed8]">{quiz.questions.length}</div>
                                    <div className="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#64748b]">Questions</div>
                                </div>
                                <div className="rounded-3xl border border-[#dbe4f0] bg-white px-4 py-5 shadow-sm">
                                    <div className="text-2xl font-bold text-[#1d4ed8]">{quiz.timeLimitMinutes}</div>
                                    <div className="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#64748b]">Minutes</div>
                                </div>
                                <div className="rounded-3xl border border-[#dbe4f0] bg-white px-4 py-5 shadow-sm">
                                    <div className="text-2xl font-bold text-[#1d4ed8]">{quiz.passingScore}%</div>
                                    <div className="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-[#64748b]">To Pass</div>
                                </div>
                            </div>

                            <div className="flex flex-col gap-3 items-center">
                                <button onClick={startQuiz} className="rounded-2xl bg-[#111827] px-8 py-3 font-bold text-white transition-all hover:-translate-y-0.5 hover:bg-[#0f172a] hover:shadow-lg hover:shadow-slate-900/20">
                                    Start Quiz
                                </button>
                                <button onClick={loadHistory} className="text-sm font-semibold text-[#1d4ed8] transition-colors hover:text-[#1e40af]">
                                    View Past Attempts
                                </button>
                            </div>
                        </div>
