...
import { Users, Plus, Trash2, X, ChevronLeft, ChevronRight, Eye, BookOpen, Clock, DollarSign } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <Button variant="outline" size="sm" onClick={openAllUsers}>
                View All
              </Button>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              ...
              <Button variant="outline" size="sm" onClick={openAllPending}>
                View All
              </Button>
            </div>
            <div className="bg-white border border-[#2d2d2d]/10">
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        By {course.instructorName || 'Unknown'} • ${course.price ?? 0}
                      </p>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-4">
              {courseDetail.thumbnailUrl && (
                <img src={courseDetail.thumbnailUrl} alt={courseDetail.title} className="w-32 h-20 object-cover rounded-lg border border-gray-200 flex-shrink-0" />
              )}
              <div className="flex-1">
                ...
                <span className="inline-block mt-2 text-xs text-orange-500 font-bold bg-orange-50 px-2 py-1 rounded">
                  {courseDetail.status}
                </span>
              </div>
            </div>
                          ...
                          {section.lessons.map((lesson: any, lIdx: number) => (
                            <div key={lesson.id || lIdx} className="px-4 py-2.5 flex items-center justify-between text-sm">
                              <div className="flex items-center gap-2">
                                ...
                                {lesson.isTrial && (
                                  <span className="text-xs bg-green-50 text-green-600 px-1.5 py-0.5 rounded font-medium">Free</span>
                                )}
                              </div>
                              <span className="text-xs text-gray-400">{lesson.durationMinutes || lesson.durationSeconds || 0}min</span>
                            </div>
                          ))}
                        </div>
                      )}
                      {(!section.lessons || section.lessons.length === 0) && (
                        <div className="px-4 py-3 text-sm text-gray-400 italic">No lessons in this section</div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-200">
              <Button onClick={() => handleApprove(courseDetail.id)} variant="primary" className="flex-1">
                Approve Course
              </Button>
              <Button onClick={() => handleReject(courseDetail.id)} variant="outline" className="flex-1">
                Reject Course
              </Button>
            </div>
          </div>
